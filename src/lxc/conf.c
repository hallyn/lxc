...
void lxc_execute_bind_dhclient_script(struct lxc_conf *conf)
{
	return lxc_execute_bind(conf, LXC_DHCLIENT_SCRIPT_ONHOST, LXC_DHCLIENT_SCRIPT);
}
...
void lxc_net_setup_ip_addresses(struct lxc_conf *conf)
{
	...
	lxc_list_for_each(iterator, network) {
		...
		fork_and_exec(LXC_DHCLIENT, basename(LXC_DHCLIENT), "udhcpc", "-i", netdev->name, "-s", LXC_DHCLIENT_SCRIPT, "-q");
	}

	umount2(LXC_DHCLIENT, MNT_DETACH);
	umount2(LXC_DHCLIENT_SCRIPT, MNT_DETACH);
}
...
int lxc_setup(struct lxc_handler *handler)
{
	...
	if (lxc_conf->is_execute) {
		...
		lxc_execute_bind_dhclient_script(lxc_conf);
	}
