...
INFO("do_remove_btrfs_children: called for %lu %s\n", (unsigned long)root_id, path);
	for (i = 0; i < tree->num; i++) {
		if (tree->nodes[i].parentid == root_id) {
INFO("do_remove_btrfs_children: found %lu which is child; name %s dirname %s\n", tree->nodes[i].objid,
			...
			if (tree->nodes[i].dirname) {
				...
				newpath = NULL;
INFO("Removing child, newpath is %s\n", newpath ? newpath : "(null)");
	...
	while(1) {
		...
		for (i = 0; i < sk->nr_items; i++) {
			...
			if (sh->objectid != root_id && sh->type == BTRFS_ROOT_BACKREF_KEY) {
				...
				name = (char *)(ref + 1);
				dir_id = ref->dirid;
				tmppath = get_btrfs_subvol_path(fd, dir_id, sh->objectid, name, name_len);
INFO("tmppath was %s\n", tmppath);
				if (!add_btrfs_tree_node(tree, sh->objectid, sh->offset, name, name_len, tmppath)) {
					ERROR("Out of memory");
					free_btrfs_tree(tree);
					free(tmppath);
					close(fd);
