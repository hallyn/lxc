...
static int systemd_jobremoved_callback(sd_bus_message *m, void *userdata, sd_bus_error *error)
{
	...
	log_trace(stderr, "got path %s unit '%s' result '%s'", path, unit, result);
...
static bool start_scope(sd_bus *bus, struct sd_callback_data *data, struct sd_event *event)
{
	...
	log_trace(stderr, "start_scope starting");
	...
	while (!data->job_complete) {
		...
		if (r == 0) // timeout
			tries++;
		log_debug(false, "bottom of loop");
	}
	log_debug(false, "after loop");
	if (!data->job_complete) {
		log_debug(false, "exit option 1");
		return log_error(false, "Error: %s job was never removed", data->scope_name);
	}
	log_debug(false, "exit option 2");
	return true;
}
...
static char *get_current_unified_cgroup(void)
{
	...
	return strdup(p);
	//return must_concat(NULL, "/sys/fs/cgroup", p, NULL);
}

// XXX delme, debugging only
static void print_current_unified_cgroup(void)
{
	__do_free char *buf = get_current_unified_cgroup();
	if (buf)
		TRACE("Current cgroup: %s", buf);
	else
		TRACE("Error getting current cgroup");
}
...
static bool enter_scope(char *scope_name, pid_t pid)
{
	...
	print_current_unified_cgroup();
	return true;
}

static bool enable_controllers_delegation(int dir, char *cg)
{
	...
	rbuf = read_file_at(dir, "cgroup.controllers", PROTECT_OPEN, 0);
	if (!rbuf)
		return false;
	...
	ret = lxc_writeat(dir, "cgroup.subtree_control", wbuf, strlen(wbuf));
	if (ret < 0)
...
{
	__do_free char *new_scope = NULL;
	__do_free char *old_scope_pids = NULL;
	__do_free char *new_scope_pids = NULL;
	__do_free char *buf = NULL;
	__do_close int pdir = -EBADF;
	int ret;

	pdir = open_at(-EBADF, parent_cgroup, O_DIRECTORY, 0, 0);
	if (pdir < 0)
		return log_error_errno(false, errno, "Failed opening cgroup dir \"%s\"", parent_cgroup);

	// Create new child cgroup 'init', and move the task there
	new_scope = must_make_path(parent_cgroup, "init", NULL);
	ret = mkdir_p(new_scope, 0755);
	if (ret < 0 && (errno != EEXIST))
		return false;
	buf = read_file_at(pdir, "cgroup.procs", PROTECT_OPEN, 0);
	if (!buf)
		return false;
	new_scope_pids = must_make_path(new_scope, "cgroup.procs", NULL);
	ret = lxc_write_to_file(new_scope_pids, buf, strlen(buf), false, 0666);
	if (ret != 0)
		return log_error_errno(false, errno, "Failed to escape to cgroup \"%s\"", new_scope);

	// enable controllers in parent_cgroup
	return enable_controllers_delegation(pdir, parent_cgroup);
}
...
#ifdef HAVE_SYSTEMD_SD_EVENT_H
static int unpriv_systemd_create(struct cgroup_ops *ops, struct lxc_conf *conf)
{
	...
	if (r < 0)
		return log_error(2, "Failed to register systemd event loop signal handler: %s", strerror(-r));

	// NEXT: create and attach event
	r = sd_event_new(&event);
	if (r < 0)
		return log_error(2, "Failed allocating new event: %s\n", strerror(-r));
	r = sd_bus_attach_event(bus, event, SD_EVENT_PRIORITY_NORMAL);
	if (r < 0) {
		// bus won't clean up event since the attach failed
		sd_event_unrefp(&event);
		return log_error(2, "Failed attaching event: %s\n", strerror(-r));
	}

	len = strlen("lxc-") + strlen(conf->name) + 4 + 6;
	full_scope_name = malloc(len + 7);
	if (!full_scope_name)
		return log_error(-ENOMEM, "Out of memory");
	...
	return 2; // failed, let's try old-school after all
}
#else // HAVE_SYSTEMD_SD_EVENT_H
static int unpriv_systemd_create(struct cgroup_ops *ops, struct lxc_conf *conf)
{
	log_trace("unpriv_systemd_create: no systemd support");
...
static bool __cgfsng_delegate_controllers(struct cgroup_ops *ops, const char *cgroup)
{
	...
	lxc_iterate_parts(cur, copy, "/") {
		...
		if (ret < 0) {
			log_debug(0, "Could not enable \"%s\" controllers in the unified cgroup %d", add_controllers, dfd_cur);
				...
				bool unprivileged, struct lxc_conf *conf)
{
	...
	log_trace(stderr, "before unpriv_systemd_create call");
	ret = unpriv_systemd_create(ops, conf);
	log_trace(stderr, "unpriv_systemd_create returned %d", ret);
	if (ret < 0)
		return ret_set_errno(false, ret);
	else if (ret == 0)
		log_info(0, "Entered an unpriv systemd scope");
